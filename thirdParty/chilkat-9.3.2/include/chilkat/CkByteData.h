...
class CkByteData : public CkObject
{
	...
	bool equals2(const void *data, unsigned long numBytes) const;
	...
	int findBytes2(const void *byteData, unsigned long byteDataLen);
	int findBytes(const CkByteData &byteData);
	bool beginsWith2(const void *byteData, unsigned long byteDataLen);
	...
	void append2(const void *byteData, unsigned long numBytes);
	...
	bool saveFile(const char *filename);
        ...
        bool loadFileW(const wchar_t *filename);
	bool saveFileW(const wchar_t *filename);

	// Create a new file, or append to an existing file.
	bool appendFile(const char *filename);
	bool appendFileW(const wchar_t *filename);
	...
	void borrowData(void *yourData, unsigned long numBytes);
